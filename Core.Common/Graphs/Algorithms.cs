...
namespace Core.Common.Graphs
{
  public static class Algorithms
  {
    ...
    public static IEnumerable<T> DfsOrder<T>(this T root, Func<T, IEnumerable<T>> successors)
    {
      ...
      return TraverseOrder(root, successors, stack.Pop, stack.Push, () => stack.Count == 0);
    }
    ...
    {
      var stack = new Stack<T>();
      return TraverseOrderWithContext(root, successors, stack.Pop, stack.Push, () => stack.Count == 0, select);
    }
    public static IEnumerable<T> BfsOrder<T>(this T root, Func<T, IEnumerable<T>> successors)
    {
      var queue = new Queue<T>();
      return TraverseOrder(root, successors, queue.Dequeue, queue.Enqueue, () => queue.Count == 0);
    }
    ...
    {
      var queue = new Queue<T>();
      return TraverseOrderWithContext(root, successors, queue.Dequeue, queue.Enqueue, () => queue.Count == 0, select);
    }
    ...
    {
      var queue = new Queue<T>();
      return TraverseOrderWithContext(root, successors, queue.Dequeue, queue.Enqueue, () => queue.Count == 0, select, initialContext);
    }
